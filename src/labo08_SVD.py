    ...
    while(len(normalizado)<len(B)):       # hasta que normalizado esté completo
        ...
        for i in range(len(normalizado),len(B)):    
            if(norma(normalizado[len(normalizado)-i],2)!=0):
                ...
                suma=suma+proyectar(canonica,normalizado[len(normalizado)-i])       # voy sumando las proyecciones del canonico en los vectores del espacio ortonormal hasta ahora
                normalizado=normalizado.append(normalizado[len(normalizado)-i]-suma)        # agrego un vector ortonormal
