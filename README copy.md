### TO DO:

- [ ] Labos 7 y 8 
- [ ] trans_afin del labo 2 
- [ ] todo lo que falta del TP 
- [ ] preguntar si se puede usar np.sum np.min, np.max, np.pow para vectorizar, np.clip, @ para teets
- [ ] preguntar si se puede usar np.clip, @ para teets 
- [ ] preguntar si esta bien que sigma sea un vector en pinvSVD
- [ ] preguntar pq el SVD de numpy devuelve mas columnas que el nuestro
- [ ] preguntar si esta bien pasar X como parametro a pinvEcuacionesNormales





